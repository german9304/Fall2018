...
int binarySearch(vector<int> av, int low,  
                  int high, int key) 
{ 
  if (high < low) 
    return -1; 
  ...
  if (key > av[mid]) 
    return binarySearch(av, (mid + 1), high, key); 
      
  // else 
    return binarySearch(av, low, (mid -1), key); 
} 
  
/* Function to get pivot. For array 3, 4, 5, 6, 1, 2 
   it returns 3 (index of 6) */
int findPivot(vector<int> av, int low, int high) 
{ 
  // base cases 
  ...
  if (high == low) return low; 
   ...
   int mid = (low + high)/2; /*low + (high - low)/2;*/
   if (mid < high && av[mid] > av[mid + 1]) 
    return mid; 
      
   if (mid > low && av[mid] < av[mid - 1]) 
    return (mid-1); 
      
   if (av[low] >= av[mid]) 
    return findPivot(av, low, mid-1); 
      
   return findPivot(av, mid + 1, high); 
} 
  
/* Searches an element key in a pivoted 
   sorted array arr[] of size n */
int searchRotated(vector<int> av, int n, int key) 
{ 
  int pivot = findPivot(av, 0, n-1); 
  
  // If we didn't find a pivot,  
  // then array is not rotated at all 
  if (pivot == -1) 
    return binarySearch(av, 0, n-1, key); 
  ...
  if (av[0] <= key){
    return binarySearch(av, 0, pivot-1, key);
    } 
      
    return binarySearch(av, pivot+1, n-1, key); 
} 
...
int main(){
    ...
    //  cout << "number : " << numbers[0] <<endl;
		    int res = searchRotated(numbers, N, M);
        cout << res <<endl;
        }else{
